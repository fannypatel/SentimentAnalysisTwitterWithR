# R Application: Sentiment Analysis of Tweets

#loading the libraries
library(shiny)
library(shinythemes)

# Define UI for R-application 
ui <- fluidPage(
  #Shiny theme selector
  shinytheme("united"),
  # Title of the Application 
  titlePanel("Sentiment Analysis of Tweets"),
  
  # Sidebar with a slider input for number of bins 
  sidebarLayout(
    sidebarPanel(
      
      textInput("searchString", "Enter Search: ","corona"), 
    ...
    # Show a plot of the generated distribution for analysis
    mainPanel(
      tabsetPanel(
...
server <- function(input, output) {
  ...
  # Authentication using consumer_key,consumer_secret,access_token,access_secret
  consumer_key <-"5yS5UYPUbeUgwTVNbJbBdweUZ"
  consumer_secret <- "U9pUmT6dHIrKVW6h8AnuMo22pTEXf4OGs0YZ35a4f73l9Q5OEM"
  access_token<-"1311715603128885250-cnPqgNDBM8GDofALWnIXift9LPEkms"
  access_secret <- "k4OhZ5HjpPImRHNJMi3aYvBucqeXVs2uZcP4VASkifW7o"
  ...
  cred <- OAuthFactory$new(consumerKey="5yS5UYPUbeUgwTVNbJbBdweUZ", consumerSecret="U9pUmT6dHIrKVW6h8AnuMo22pTEXf4OGs0YZ35a4f73l9Q5OEM",requestURL="https://api.twitter.com/oauth/request_token",accessURL="https://api.twitter.com/oauth/access_token",authURL="https://api.twitter.com/oauth/authorize")
  ...
  {
    sample=NULL  #Initializing  
    #We can get the text from df$text, which are the cleaned tweets
...
# Run the application using shinyApp
